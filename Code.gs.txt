      ...
      pageTitle = 'Monthly Stats Dashboard';
        ...
        if (!isUserAuthorizedForProcessing()) {
            const deniedHtml = `
                ...
                <style>body { display: flex; align-items: center; justify-content: center; height: 100vh; background-color: #f8f9fa; text-align: center; }</style>
...
function getConfigOptionsForAdmin() {
    ...
    try {
        ...
        return data.slice(1).map((row, index) => ({
            ...
            displayOrder: orderCol > -1 && row.length > orderCol ? (row[orderCol] || '') : '',
    ...
    try {
        ...
        const resultOrders = paginatedRows.map(row => {
            ...
            const orderResult = {
                ...
                outfitOrdered: indices.OutfitOrdered !== undefined && row.length > indices.OutfitOrdered ? row[indices.OutfitOrdered] : "",
                omsId: row[indices.OMS_ID],
                isDuplicate: emailForDupCheck ? (emailCounts[emailForDupCheck] > 1) : false
            };
            return orderResult;
        });
          ...
          .forEach(r => {
            ...
            if (!outfits[oName]) {
                outfits[oName] = {
                    ...
                    isActive: idx.IS_ACTIVE > -1 && r.length > idx.IS_ACTIVE ? r[idx.IS_ACTIVE] === true : true,
...
function updateInventoryItems(collectionName, outfitName, piecesToUpdate, imageData = null, newIsActiveStatus = null, newOutfitQuickNote = null) {
    ...
    try {
        ...
        if (updatesMadeCounter > 0) {
            ...
            return { success: true, message: `Inventory updated successfully for ${outfitName}.`, finalImageId: newImageFileId }; // Return finalImageId
        }
        return { success: true, message: "No changes were detected for the inventory.", finalImageId: newImageFileId }; // Return finalImageId even if no other changes
...
/**
 * Synchronizes outfit details (pieces, active status, image, quick note) across all active collections
 * where the outfit already exists. It does NOT create the outfit in active collections where it's missing.
 * @param {object} outfitDataFromClient Object containing canonical outfit details.
 * Expected properties:
 * - originalCollectionName (string): The collection where the edit was initiated.
 * - outfitName (string): The name of the outfit being synced.
 * - newIsActiveStatus (boolean): The desired active status for the outfit.
 * - newOutfitQuickNote (string): The desired quick note.
 * - finalImageUrl (string|null): The File ID of the image to use for the outfit, or null if no specific image to sync.
 * - pieces (Array<object>): The definitive list of piece objects for this outfit. Each piece object should have:
 * { productName, sku, category, color, availableSizes: [], eta, notes, swapSku, swapAvailableSizes: [], sequence }
 * @return {object} Result object {success: boolean, message: string}
 */
function syncOutfitAcrossActiveCollections(outfitDataFromClient) {
  if (!isUserAuthorizedForProcessing()) {
    logActivity("OUTFIT_SYNC_DENIED", `Unauthorized attempt to sync outfit: ${outfitDataFromClient ? outfitDataFromClient.outfitName : 'Unknown'}`);
    return { success: false, message: "Error: Not authorized to sync outfits." };
  }

  if (!inventorySheet) {
    logError('syncOutfitAcrossActiveCollections', 'Inventory sheet (Active SS) not found.');
    return { success: false, message: "Inventory sheet missing." };
  }
  if (!outfitDataFromClient || !outfitDataFromClient.outfitName || !Array.isArray(outfitDataFromClient.pieces)) {
    logError('syncOutfitAcrossActiveCollections', 'Invalid outfitDataFromClient received for sync.', JSON.stringify(outfitDataFromClient));
    return { success: false, message: "Invalid data received for sync (missing outfit name or pieces)." };
  }

  Logger.log(`SYNC: Starting sync for Outfit: "${outfitDataFromClient.outfitName}" initiated from Collection: "${outfitDataFromClient.originalCollectionName}"`);

  try {
    let imageIdToPropagate = outfitDataFromClient.finalImageUrl || null; // This is the key: client sends the image ID decided during the modal edit.

    // If new base64 image data was sent (e.g., user uploaded a new image *during the sync action specifically*, which is unlikely with current UI but good to handle)
    // This part is more relevant if the "sync" button itself could trigger an image upload.
    // For now, we assume `finalImageUrl` is the one to use, potentially set by a prior `updateInventoryItems` call.
    if (outfitDataFromClient.imageData && outfitDataFromClient.imageData.base64Data) {
        Logger.log(`SYNC: New image data explicitly provided for SYNC operation for ${outfitDataFromClient.outfitName}. Saving to Drive...`);
        const savedImgId = saveImageToDrive(outfitDataFromClient.imageData, outfitDataFromClient.originalCollectionName);
        if (savedImgId) {
            imageIdToPropagate = savedImgId; // Override with newly uploaded image for sync
            Logger.log(`SYNC: New image for sync saved with ID: ${imageIdToPropagate}. This will be propagated.`);
        } else {
            Logger.log(`SYNC: WARN - Failed to save new image explicitly provided for sync of ${outfitDataFromClient.outfitName}.`);
        }
    }


    const activeCollectionsResult = getActiveCollections();
    if (!activeCollectionsResult || activeCollectionsResult.length === 0) {
        logActivity("OUTFIT_SYNC_FAIL", `No active collections found to sync outfit: ${outfitDataFromClient.outfitName}`);
        return { success: false, message: "No active collections available to sync to." };
    }
    const activeCollectionNames = activeCollectionsResult.map(c => c.name);

    const inventorySheetData = inventorySheet.getDataRange().getValues();
    const inventoryHeaders = inventorySheetData[0];
    const headerMap = {};
    inventoryHeaders.forEach((h, i) => { if(h) headerMap[h.toString().trim()] = i; });

    // Validate essential headers
    const essentialHeadersForSync = [
        INV_HEADERS.COLLECTION, INV_HEADERS.OUTFIT_NAME, INV_HEADERS.IMAGE_URL, INV_HEADERS.IS_ACTIVE,
        INV_HEADERS.PIECE_SEQ, INV_HEADERS.PROD_NAME, INV_HEADERS.PROD_SKU, INV_HEADERS.COLOR,
        INV_HEADERS.AVAIL_SIZES, INV_HEADERS.ETA, INV_HEADERS.SWAP_SKU, INV_HEADERS.SWAP_SIZES, INV_HEADERS.NOTES
    ];
    if (INV_HEADERS.OUTFIT_QUICK_NOTE) essentialHeadersForSync.push(INV_HEADERS.OUTFIT_QUICK_NOTE);

    for (const headerKey of essentialHeadersForSync) {
        if (headerMap[headerKey] === undefined && headerKey !== INV_HEADERS.OUTFIT_QUICK_NOTE) {
            const errorMsg = `Inventory sheet is missing critical header for sync: ${headerKey}.`;
            logError('syncOutfitAcrossActiveCollections', errorMsg);
            return { success: false, message: `${errorMsg} Sync aborted.` };
        }
    }

    const newInventorySheetRows = [inventoryHeaders];
    const collectionsWhereOutfitWasRebuilt = new Set();

    // Iterate through existing inventory to rebuild or preserve rows
    for (let i = 1; i < inventorySheetData.length; i++) {
      const currentRow = inventorySheetData[i];
      const currentRowCollectionName = currentRow[headerMap[INV_HEADERS.COLLECTION]];
      const currentRowOutfitName = currentRow[headerMap[INV_HEADERS.OUTFIT_NAME]];
      const currentRowImageUrl = currentRow[headerMap[INV_HEADERS.IMAGE_URL]] || "";

      if (currentRowOutfitName === outfitDataFromClient.outfitName && activeCollectionNames.includes(currentRowCollectionName)) {
        // This is an instance of the target outfit in an ACTIVE collection. It needs to be rebuilt.
        if (!collectionsWhereOutfitWasRebuilt.has(currentRowCollectionName)) {
          // Only rebuild once per collection.
          Logger.log(`SYNC: Rebuilding outfit "${outfitDataFromClient.outfitName}" in ACTIVE collection "${currentRowCollectionName}".`);
          outfitDataFromClient.pieces.forEach((pieceData, pieceIndex) => {
            const newRow = new Array(inventoryHeaders.length).fill('');
            newRow[headerMap[INV_HEADERS.COLLECTION]] = currentRowCollectionName;
            newRow[headerMap[INV_HEADERS.OUTFIT_NAME]] = outfitDataFromClient.outfitName;
            newRow[headerMap[INV_HEADERS.IMAGE_URL]] = imageIdToPropagate !== null ? imageIdToPropagate : currentRowImageUrl; // Use new/synced image if available, else keep this instance's image
            newRow[headerMap[INV_HEADERS.IS_ACTIVE]] = outfitDataFromClient.newIsActiveStatus;
            if (headerMap[INV_HEADERS.OUTFIT_QUICK_NOTE] !== undefined) {
                newRow[headerMap[INV_HEADERS.OUTFIT_QUICK_NOTE]] = outfitDataFromClient.newOutfitQuickNote || "";
            }

            newRow[headerMap[INV_HEADERS.PIECE_SEQ]] = pieceData.sequence !== undefined ? pieceData.sequence : (pieceIndex + 1) * 10;
            newRow[headerMap[INV_HEADERS.PROD_NAME]] = pieceData.productName || "";
            newRow[headerMap[INV_HEADERS.PROD_SKU]] = pieceData.sku || "";
            newRow[headerMap[INV_HEADERS.COLOR]] = pieceData.color || "";
            let etaValue = pieceData.eta || "";
            if (etaValue && typeof etaValue === 'string' && etaValue.match(/^\d{4}-\d{2}-\d{2}$/)) { // Basic YYYY-MM-DD check
                try { etaValue = new Date(etaValue); } catch(e) { /* keep as string if parse fails */ }
            }
            newRow[headerMap[INV_HEADERS.ETA]] = etaValue;
            newRow[headerMap[INV_HEADERS.NOTES]] = pieceData.notes || "";
            newRow[headerMap[INV_HEADERS.SWAP_SKU]] = pieceData.swapSku || "";
            newRow[headerMap[INV_HEADERS.AVAIL_SIZES]] = Array.isArray(pieceData.availableSizes) ? pieceData.availableSizes.join(',') : (pieceData.availableSizes || "");
            newRow[headerMap[INV_HEADERS.SWAP_SIZES]] = Array.isArray(pieceData.swapAvailableSizes) ? pieceData.swapAvailableSizes.join(',') : (pieceData.swapAvailableSizes || "");

            newInventorySheetRows.push(newRow);
          });
          collectionsWhereOutfitWasRebuilt.add(currentRowCollectionName);
        }
        // Do not add the original rows for this outfit in this active collection to newInventorySheetRows, as they are replaced.
      } else {
        // This row is for a different outfit OR for the target outfit in an INACTIVE collection. Keep it as is.
        newInventorySheetRows.push(currentRow);
      }
    }
    
    const outfitsSyncedCount = collectionsWhereOutfitWasRebuilt.size;

    if (outfitsSyncedCount > 0) {
        inventorySheet.clearContents(); // Clear existing content including headers
        inventorySheet.getRange(1, 1, newInventorySheetRows.length, newInventorySheetRows[0].length).setValues(newInventorySheetRows);
        SpreadsheetApp.flush();
        Logger.log(`SYNC: Successfully wrote ${newInventorySheetRows.length -1} total rows back to inventory sheet after sync.`);
    } else {
        Logger.log(`SYNC: No active collections found containing the outfit "${outfitDataFromClient.outfitName}" to sync updates to (beyond potentially the original collection if it was active).`);
         // If the original collection was active and processed, outfitsSyncedCount would be at least 1.
         // If outfitsSyncedCount is 0, it means the original collection was not active, or no other active collections had the outfit.
         // The primary update for the original collection is handled by `updateInventoryItems`.
         // This sync function's main job is propagation. If no propagation targets, it's fine.
    }

    logActivity("OUTFIT_SYNC_ACROSS_COLLECTIONS", `Synced outfit: ${outfitDataFromClient.outfitName}`, {
      outfit: outfitDataFromClient.outfitName,
      originalCollection: outfitDataFromClient.originalCollectionName,
      collectionsAffectedBySyncCount: outfitsSyncedCount,
      processedActiveCollections: Array.from(collectionsWhereOutfitWasRebuilt),
      propagatedIsActive: outfitDataFromClient.newIsActiveStatus,
      propagatedQuickNote: !!outfitDataFromClient.newOutfitQuickNote,
      propagatedImageId: imageIdToPropagate,
      canonicalPieceCount: outfitDataFromClient.pieces.length
    });

    return { success: true, message: `Outfit '${outfitDataFromClient.outfitName}' changes propagated to ${outfitsSyncedCount} active collection(s).` };

  } catch (e) {
    logError('syncOutfitAcrossActiveCollections', `Error syncing outfit ${outfitDataFromClient.outfitName}: ${e.message}`, e.stack);
    return { success: false, message: `Error syncing outfit: ${e.message}` };
  }
}
...
function archiveCollection(collectionNameToArchive) {
  ...
  try {
    ...
    try {
        ...
        SpreadsheetApp.flush();
        logError('archiveCollection', `Successfully archived collection: ${collectionNameToArchive}`, 'INFO'); // Using logError with INFO type
...
function uploadRawDataToSheet(fileContent, fileName) {
    ...
    try {
        ...
        if (numRowsToWrite > 0) {
            ...
            return { success: true, message: `Successfully uploaded ${numRowsToWrite} data rows from "${fileName}" to RawImport sheet (Active SS). Ready to process.` };
        } else {
